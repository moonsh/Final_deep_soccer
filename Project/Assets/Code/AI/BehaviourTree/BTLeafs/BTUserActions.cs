...
public class BTUserActions : BTNode
{
    public override BTResult Execute()
    {
        ...
        if (userAction.Equals("Move"))
        {
            ...
            if (!(context.navAgent.GetComponent<Transform>().position.x <= destination.x + 1 && context.navAgent.GetComponent<Transform>().position.x >= destination.x - 1) ||
                !(context.navAgent.GetComponent<Transform>().position.z <= destination.z + 1 && context.navAgent.GetComponent<Transform>().position.z >= destination.z - 1))
            {
